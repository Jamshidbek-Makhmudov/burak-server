...
class MemberService {
    ...
    async getRestaurant() {
        const result = await this.memberModel
            ...
            .exec();
        if (!result)
            throw new Errors_1.default(Errors_1.HttpmCode.NOT_FOUND, Errors_1.Message.NO_DATA_FAUND);
        return result;
    }
    async signup(input) {
        ...
        try {
            ...
            throw new Errors_1.default(Errors_1.HttpmCode.BAD_REQUEST, Errors_1.Message.USED_NICK_PHONE);
        }
    }
    async login(input) {
        ...
        const member = await this.memberModel
            ...
            .exec();
        if (!member)
            throw new Errors_1.default(Errors_1.HttpmCode.NOT_FOUND, Errors_1.Message.NO_MEMBER_NICK);
        else if (member.memberStatus === member_enum_1.MemberStatus.BLOCK) {
            throw new Errors_1.default(Errors_1.HttpmCode.FORBIDDEN, Errors_1.Message.BLOCKED_USER);
        }
        const isMatch = await bcrypt.compare(input.memberPassword, member.memberPassword);
        if (!isMatch)
            throw new Errors_1.default(Errors_1.HttpmCode.UNAUTHORIZED, Errors_1.Message.WRONG_PASWORD);
    ...
    async updateMember(member, input) {
        ...
        const result = await this.memberModel
            ...
            .exec();
        if (!result)
            throw new Errors_1.default(Errors_1.HttpmCode.NOT_FOUND, Errors_1.Message.UPDATE_FAILED);
        return result;
    }
    async getTopUsers() {
        const result = await this.memberModel
    ...
    async processSignup(input) {
        const exist = await this.memberModel
            ...
            .exec();
        if (exist)
            throw new Errors_1.default(Errors_1.HttpmCode.BAD_REQUEST, Errors_1.Message.CREATE_FAILED);
        const salt = await bcrypt.genSalt();
        input.memberPassword = await bcrypt.hash(input.memberPassword, salt);
        try {
            const result = await this.memberModel.create(input);
    ...
    async updateChoseUser(input) {
        ...
        const result = await this.memberModel
            ...
            .exec();
        if (!result)
            throw new Errors_1.default(Errors_1.HttpmCode.NOT_MODIFIED, Errors_1.Message.UPDATE_FAILED);
        return result;
    }
}
